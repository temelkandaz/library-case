...
export const validateBorrowBookInput = [
    param('userId').notEmpty().withMessage('userId is required').isInt().withMessage('userId must be an integer!'),
    param('bookId').notEmpty().withMessage('bookId is required').isInt().withMessage('bookId must be an integer!')
];

export const validateReturnBookInput = [
    param('userId').notEmpty().withMessage('userId is required').isInt().withMessage('userId must be an integer!'),
    param('bookId').notEmpty().withMessage('bookId is required').isInt().withMessage('bookId must be an integer!'),
    check('score').notEmpty().withMessage('score is required!').isInt({ min: 1, max: 10}).withMessage('score must be an integer between 1 and 10!')
];
