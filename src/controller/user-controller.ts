...
class UserController {
  ...
  async getUserById(req: Request, res: Response, next: NextFunction) {
    ...
    const borrowedRecords = records.filter(
      (record) => record.status == RecordStatus.BORROWED,
    );
    const returnedRecords = records.filter(
      (record) => record.status == RecordStatus.RETURNED,
    );
  ...
  @catchError
  async borrowBookByUserIdAndBookId(
    req: Request,
    res: Response,
    next: NextFunction,
  ) {
  ...
  @catchError
  async returnBookByUserIdAndBookIdAndScore(
    req: Request,
    res: Response,
    next: NextFunction,
  ) {
